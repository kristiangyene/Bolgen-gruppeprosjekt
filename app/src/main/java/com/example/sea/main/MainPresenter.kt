...
class MainPresenter(view: MainContract.View, private var context: Context, private var interactor: MainContract.Interactor) : MainContract.Presenter {
...
//            recreate()
    ...
    override fun requestPermission(permissionOption: String) {
        when (permissionOption) {
            "sms" -> {
                ActivityCompat.requestPermissions(view!!.getActivity(), arrayOf(Manifest.permission.SEND_SMS), SMS_PERMISSION)
            }
            "location" -> {
                ActivityCompat.requestPermissions(view!!.getActivity(), arrayOf(Manifest.permission.ACCESS_COARSE_LOCATION), LOCATION_PERMISSION)
            }
            else -> {
                ActivityCompat.requestPermissions(view!!.getActivity(), arrayOf(Manifest.permission.SEND_SMS, Manifest.permission.ACCESS_COARSE_LOCATION), BOTH_PERMISSION)
            }
        }
    }
